import sys
sys.path.extend(['/root/scrabble-gan'])
...
from src.bigacgan.data_utils import load_prepare_data, train, make_gif, load_random_word_list, load_lines_data
...
@gin.configurable('io')
def setup_io(lines, base_path, checkpoint_dir, gen_imgs_dir, model_dir, raw_dir, read_dir, input_dim, buf_size, n_classes,
    ...
    return lines, input_dim, buf_size, n_classes, seq_len, bucket_size, ckpt_path, gen_path, m_path, raw_dir, read_dir, char_vec
...
def main():
    ...
    lines, in_dim, buf_size, n_classes, seq_len, bucket_size, ckpt_path, gen_path, m_path, raw_dir, read_dir, char_vec = setup_io()
    ...
    if not lines:
        ...
        train_dataset = load_lines_data(in_dim, batch_size, read_dir, char_vec, bucket_size)
    ...
    # start training
    train(train_dataset, generator, discriminator, recognizer, gan, checkpoint, checkpoint_prefix, manager,
          generator_optimizer,
