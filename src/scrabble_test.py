...
@gin.configurable('io')
def setup_io(base_path, checkpoint_dir, gen_imgs_dir, gen_data_dir, model_dir, raw_dir, read_dir, input_dim, buf_size,
             n_classes,
...
def main():
    ...
    gin.parse_config_file(
        '/Users/jsoncunanan/Documents/GitHub/scrabble-gan/src/scrabble_gan.gin')  # '/root/scrabble-gan/src/scrabble_gan.gin'
    ...
    def make_dataset(num_gen, num_batch=20):
        lines = []
        dic = {}
        for j in trange(num_batch):
            ...
            random_bucket_idx = random.randint(4, bucket_size - 1)
            labels = np.array([random.choice(random_words[random_bucket_idx]) for _ in range(20)], np.int32)
            test_input = [seed[:20], labels]

            preds = generator(test_input, training=False)
            preds = (preds + 1) / 2.0

            for i in range(preds.shape[0]):
                name = str("".join([char_vec[label] for label in labels[i]]))
                filename = str(j) + '_' + str(i)
                # save label to json file
                dic["answer"] = name
                with open(gen_data_path + filename+'.json', 'w') as outfile:
                    json.dump(dic, outfile, indent=3)
                tf.keras.preprocessing.image.save_img(gen_data_path + filename + ".png", preds[i])
                lines.append('test/' + filename + '.png\t' + name + '\n')
        # for lmdb use
